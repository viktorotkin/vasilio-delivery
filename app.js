...
async function renderCatalog(){var c=document.getElementById("catalog");try{c.innerHTML='<div class="loading">\u0417\u0430\u0433\u0440\u0443\u0437\u043a\u0430...</div>';var items=await fetchProducts();console.log("[catalog] "+(items?items.length:0)+" items");if(!items||items.length===0){c.innerHTML='<div class="loading">\u0422\u043e\u0432\u0430\u0440\u044b \u0432\u0440\u0435\u043c\u0435\u043d\u043d\u043e \u043e\u0442\u0441\u0443\u0442\u0441\u0442\u0432\u0443\u044e\u0442.</div>';return;}c.innerHTML="";items.filter(function(i){return i.in_stock;}).slice(0,24).forEach(function(i){var d=document.createElement("div");d.className="card";var img=i.photo_url?'<img src="https://images.weserv.nl/?url=vlvc.ru'+i.photo_url+'" loading="lazy" alt="'+i.name+'" onerror="this.style.display=\'none\'">':"";var badges='<span class="badge">'+(i.category||"\u041f\u0440\u043e\u0434\u0443\u043a\u0446\u0438\u044f")+"</span>";if(i.brand)badges+='<span class="badge">'+i.brand+"</span>";var price='<div class="price">'+i.price+"\u20bd</div>";var old=i.old_price?'<span class="old">'+i.old_price+"\u20bd</span>":"";var safeName=(i.name||"").replace(/'/g,"").replace(/"/g,"&quot;");var btn='<button class="btn" onclick="addToCart('+i.id+",&quot;"+safeName+"&quot;,"+i.price+')">\u0412 \u043a\u043e\u0440\u0437\u0438\u043d\u0443</button>';d.innerHTML=img+"<h3>"+i.name+"</h3>"+badges+"<p>"+(i.description||"")+"</p>"+price+old+btn;c.appendChild(d);});}catch(e){console.error("[ERROR]",e);c.innerHTML='<div class="loading">\u041e\u0448\u0438\u0431\u043a\u0430: '+e.message+'</div>';}}
renderCatalog();
